...
class TestTransactions(APITestCase):
    ...
    def test_user_can_make_a_deposit_fails(self):
        ...
        # asset after deposit
        eq_(response.status_code, status.HTTP_400_BAD_REQUEST)
        eq_(FAILURE_MSGS["must_be_greater"].format("Deposit", 0), json.loads(response.content).get("amount")[0])
    ...
    def test_user_can_make_a_withdrawal_fails(self):
        ...
        balance.book_balance = 100
        balance.available_balance = 100
        balance.save()

        payload = {"amount": balance.available_balance + 10}
        response = self.client.post(self.withdrawal_url, payload)
        eq_(response.status_code, status.HTTP_400_BAD_REQUEST)
